...
def test_isi():
    ...
    expected_isi_val = sum(
        (expected_times[1:] - expected_times[:-1]) * expected_isi
    ) / (expected_times[-1] - expected_times[0])
...
def test_spike():
    ...
    y_all = np.array(
        [
            ...
            0.555555555555555580,
            0.222222222222222210,
            0.305555555555555580,
            0.255102040816326536,
            0.000000000000000000,
            0.000000000000000000,
            0.255102040816326536,
            0.255102040816326536,
    ...
    s2 = np.array(
        [
            ...
            (0.1 * 0.05 * 2) / 0.15,
            0.05,
            (0.05 * 0.2 + 0.1 * 0.15) / 0.35,
            (0.05 * 0.1 + 0.1 * 0.25) / 0.35,
            0.1,
            0.1,
            0.05,
            0.05,
    ...
    expected_spike_val = sum(
        (expected_times[1:] - expected_times[:-1]) * (expected_y1 + expected_y2) / 2
    )
    ...
    assert_almost_equal(spk.spike_distance(t1, t2), expected_spike_val, decimal=15)
    ...
    s2_r = np.array([0.1, 0.1 * 0.3 / 0.3, 0.1 * 0.2 / 0.3, 0.0, 0.1, 0.0, 0.0])
    s2_l = np.array([0.1, 0.1 * 0.3 / 0.3, 0.1 * 0.2 / 0.3, 0.0, 0.1, 0.0, 0.0])
    ...
    assert_almost_equal(spk.spike_distance(t1, t2), expected_spike_val, decimal=16)
...
def test_spike_sync():
    ...
    assert_almost_equal(spk.spike_sync(spikes1, spikes2, max_tau=0.05), 0.0, decimal=16)
...
def test_multi_isi():
    check_multi_profile(spk.isi_profile, spk.isi_profile_multi, spk.isi_distance_multi)


def test_multi_spike():
    check_multi_profile(
        spk.spike_profile, spk.spike_profile_multi, spk.spike_distance_multi
    )
...
def test_multi_spike_sync():
    ...
    assert_almost_equal(
        spk.spike_sync_multi([spikes1, spikes2, spikes3]), expected, decimal=15
    )
    ...
    assert_almost_equal(spk.spike_sync_profile_multi(sts).avrg(), 1.0 / 6.0, decimal=15)


def check_dist_matrix(dist_func, dist_matrix_func, Diagonal=0.0):
    # generate spike trains:
    t1 = SpikeTrain([0.2, 0.4, 0.6, 0.7], 1.0)
    t2 = SpikeTrain([0.3, 0.45, 0.8, 0.9, 0.95], 1.0)
    t3 = SpikeTrain([0.2, 0.4, 0.6], 1.0)
...
def test_spike_sync_matrix():
    check_dist_matrix(spk.spike_sync, spk.spike_sync_matrix, Diagonal=1.0)
