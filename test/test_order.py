...
def test_spike_order():
    ...
    assert_almost_equal(spk.spike_order(st1, st2), 2.0/3.0)
    assert_almost_equal(spk.spike_order(st1, st2, normalize=False),
                        2.0)

    # exchange order of spike trains should give exact negative profile
    assert_almost_equal(spk.spike_order(st2, st1), -2.0/3.0)
    assert_almost_equal(spk.spike_order(st2, st1, normalize=False),
                        -2.0)

    st3 = SpikeTrain([105, 195, 500], [0, 1000])
    assert_almost_equal(spk.spike_order(st1, st3), 0.0)
    assert_almost_equal(spk.spike_order(st1, st3, normalize=False),
                        0.0)
    assert_almost_equal(spk.spike_order(st3, st1), 0.0)

    D = spk.spike_order_matrix([st1, st2, st3], normalize=False)
    D_expected = np.array([[0, 2.0, 0.0], [-2.0, 0.0, -1.0], [0.0, 1.0, 0.0]])
    assert_array_equal(D, D_expected)

    dir_profs = spk.spike_order_values([st1, st2, st3])
...
def test_spike_train_order():
    ...
    D_matrix = spk.spike_order_matrix([st1, st2, st3], normalize=False)
