...
def test_spike_directionality():
    ...
    assert_almost_equal(spk.spike_directionality(st1, st2, normalize=False), 2.0)
    ...
    assert_almost_equal(spk.spike_directionality(st2, st1, normalize=False), -2.0)

    st3 = SpikeTrain([105, 195, 500], [0, 1000])
    assert_almost_equal(spk.spike_directionality(st1, st3), 0.0)
    assert_almost_equal(spk.spike_directionality(st1, st3, normalize=False), 0.0)
...
def test_spike_train_order():
    ...
    assert f.almost_equal(DiscreteFunc(expected_x12, expected_y12, expected_mp12))
    ...
    assert f.almost_equal(DiscreteFunc(expected_x23, expected_y23, expected_mp23))
