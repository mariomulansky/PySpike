# Module containing functions to compute the SPIKE order and the
...
def spike_order_values(*args, **kwargs):
    ...
    if len(args) == 1:
        ...
        return _spike_order_values_impl(args, **kwargs)


def _spike_order_values_impl(spike_trains, indices=None,
...
def spike_train_order(spike_train1, spike_train2, normalize=True,
    ...
    if interval is None:
        ...
        try:
            ...
            # use profile.
            d1, x = spike_order_values([spike_train1, spike_train2],
                                                 interval=interval,
                                                 max_tau=max_tau,
                                                 MRTS=MRTS)
            d = 2*np.sum(d1)
...
def _optimal_spike_train_sorting_from_matrix(D, full_output=False):
    ...
    :param D: The order (Spike-ORDER) matrix.
...
def optimal_spike_train_sorting(spike_trains,  indices=None, interval=None,
    ...
    order matrix and optimize the order using simulated annealing.
