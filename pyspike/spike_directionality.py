...
def _spike_directionality_values_impl(
    spike_trains, indices=None, interval=None, max_tau=None, **kwargs
):
...
def spike_directionality(
    spike_train1, spike_train2, normalize=True, interval=None, max_tau=None, **kwargs
):
...
############################################################
def spike_directionality_matrix(
    spike_trains, normalize=True, indices=None, interval=None, max_tau=None, **kwargs
):
...
############################################################
def spike_train_order_profile_bi(spike_train1, spike_train2, max_tau=None, **kwargs):
    ...
    try:
        from .cython.cython_directionality import (
            spike_train_order_profile_cython as spike_train_order_profile_impl,
        )
    except ImportError:
    ...
    times, coincidences, multiplicity = spike_train_order_profile_impl(
        spike_train1.spikes,
        spike_train2.spikes,
        spike_train1.t_start,
        spike_train1.t_end,
...
############################################################
def spike_train_order_profile_multi(spike_trains, indices=None, max_tau=None, **kwargs):
...
############################################################
def _spike_train_order_impl(
    spike_train1, spike_train2, interval=None, max_tau=None, **kwargs
):
    ...
    if interval is None:
        ...
        try:
            ...
            c, mp = spike_train_order_profile(
                spike_train1, spike_train2, max_tau=max_tau, MRTS=MRTS
            ).integral(interval)
        return c, mp
    else:
        # some specific interval is provided: not yet implemented
...
def spike_train_order_bi(
    ...
    :returns: The spike train order value (Synfire Indicator)
    """
    c, mp = _spike_train_order_impl(
        spike_train1, spike_train2, interval, max_tau, **kwargs
    )
    if normalize:
        return 1.0 * c / mp
    else:
...
############################################################
def spike_train_order_multi(
    spike_trains, indices=None, normalize=True, interval=None, max_tau=None, **kwargs
):
    """Computes the overall spike train order value (Synfire Indicator)
    ...
    m_total = 0.0
    for i, j in pairs:
        e, m = _spike_train_order_impl(
            spike_trains[i], spike_trains[j], interval, max_tau, MRTS=MRTS, RI=RI
        )
        e_total += e
        m_total += m
...
############################################################
def optimal_spike_train_sorting(
    spike_trains, indices=None, interval=None, max_tau=None, full_output=False, **kwargs
):
