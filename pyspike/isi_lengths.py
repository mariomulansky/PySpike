...
def isi_lengths(spike_times, t_start, t_end):
    ...
    if spike_times[0] > t_start:
        del_start = (
            max(spike_times[0] - t_start, spike_times[1] - spike_times[0])
            if N > 1
            else spike_times[0] - t_start
        )
        i_start = 0
    else:
        del_start = (
            spike_times[1] - spike_times[0] if N > 1 else t_start - spike_times[0]
        )
        i_start = 1

    if spike_times[-1] < t_end:
        del_end = (
            max(t_end - spike_times[-1], spike_times[-1] - spike_times[-2])
            if N > 1
            else t_end - spike_times[0]
        )
        i_end = N
    else:
        del_end = spike_times[-1] - spike_times[-2] if N > 1 else spike_times[0] - t_end
        i_end = N - 1
