...
def spike_sync_profile_bi(spike_train1, spike_train2, max_tau=None, **kwargs):
    ...
    try:
        from .cython.cython_profiles import (
            coincidence_profile_cython as coincidence_profile_impl,
        )
    except ImportError:
        pyspike.NoCythonWarn()

        # use python backend
        from .cython.python_backend import (
            coincidence_python as coincidence_profile_impl,
        )

    if max_tau is None:
        max_tau = 0.0

    times, coincidences, multiplicity = coincidence_profile_impl(
        spike_train1.spikes,
        spike_train2.spikes,
    ...
    if interval is None:
        ...
        try:
            from .cython.cython_distances import (
                coincidence_value_cython as coincidence_value_impl,
            )

            if max_tau is None:
                max_tau = 0.0
            c, mp = coincidence_value_impl(
                spike_train1.spikes,
                spike_train2.spikes,
                spike_train1.t_start,
                spike_train1.t_end,
            ...
            return spike_sync_profile_bi(
                spike_train1, spike_train2, max_tau, **kwargs
            ).integral(interval)
    else:
...
def spike_sync_multi(spike_trains, indices=None, interval=None, max_tau=None, **kwargs):
    ...
    mp = 0.0
    for i, j in pairs:
        c, m = _spike_sync_values(
            spike_trains[i], spike_trains[j], interval, max_tau, **kwargs
        )
        coincidence += c
        mp += m
...
def spike_sync_matrix(
    ...
    ShouldBeSync = _generic_distance_matrix(
        spike_trains, dist_func, indices, interval, **kwargs
    )
...
def filter_by_spike_sync(
    ...
    **kwargs,
    ...
    try:
        ...
        # use python backend
        from .cython.python_backend import coincidence_single_python as coincidence_impl
    ...
    for i, st in enumerate(spike_trains):
        ...
        for j in range(N):
            if i == j:
                continue
            coincidences += coincidence_impl(
                st.spikes, spike_trains[j].spikes, st.t_start, st.t_end, max_tau, MRTS
            )
        filtered_spikes = st[coincidences > threshold * (N - 1)]
        filtered_spike_trains.append(
            SpikeTrain(filtered_spikes, [st.t_start, st.t_end])
        )
        if return_removed_spikes:
            removed_spikes = st[coincidences <= threshold * (N - 1)]
            removed_spike_trains.append(
                SpikeTrain(removed_spikes, [st.t_start, st.t_end])
            )
    if return_removed_spikes:
