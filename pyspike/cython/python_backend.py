...
# isi_distance_python
############################################################
def isi_distance_python(s1, s2, t_start, t_end, MRTS=0.0):
    ...
    while index1 + index2 < N1 + N2 - 2:
        ...
        if (index1 < N1 - 1) and (index2 == N2 - 1 or s1[index1 + 1] < s2[index2 + 1]):
            ...
            if index1 < N1 - 1:
                ...
                # edge correction
                nu1 = (
                    max(t_end - s1[N1 - 1], s1[N1 - 1] - s1[N1 - 2])
                    if N1 > 1
                    else t_end - s1[N1 - 1]
                )

        elif (index2 < N2 - 1) and (
            index1 == N1 - 1 or s1[index1 + 1] > s2[index2 + 1]
        ):
            index2 += 1
            ...
            if index2 < N2 - 1:
                ...
                # edge correction
                nu2 = (
                    max(t_end - s2[N2 - 1], s2[N2 - 1] - s2[N2 - 2])
                    if N2 > 1
                    else t_end - s2[N2 - 1]
                )
        ...
        isi_values[index] = abs(nu1 - nu2) / max([nu1, nu2, MRTS])
        index += 1
...
############################################################
def spike_distance_python(spikes1, spikes2, t_start, t_end, MRTS=0.0, RI=False):
    ...
    while index1 + index2 < N1 + N2 - 2:
        ...
        if (index1 < N1 - 1) and (t_f1 < t_f2 or index2 == N2 - 1):
            ...
            if index1 < N1 - 1:
                ...
                dt_f1 = dt_p1
                isi1 = (
                    max(t_end - t1[N1 - 1], t1[N1 - 1] - t1[N1 - 2])
                    if N1 > 1
                    else t_end - t1[N1 - 1]
                )
            ...
            if index2 < N2 - 1:
                ...
                dt_f2 = dt_p2
                isi2 = (
                    max(t_end - t2[N2 - 1], t2[N2 - 1] - t2[N2 - 2])
                    if N2 > 1
                    else t_end - t2[N2 - 1]
                )
...
def get_tau(spikes1, spikes2, i, j, max_tau, MRTS):
    ...
    mF1, mF2, mP1, mP2 = mF1 / 2.0, mF2 / 2.0, mP1 / 2.0, mP2 / 2.0
    MRTS /= 4
...
# coincidence_python
############################################################
def coincidence_python(spikes1, spikes2, t_start, t_end, max_tau, MRTS=0.0):
...
############################################################
def coincidence_single_python(spikes1, spikes2, t_start, t_end, max_tau, MRTS=0.0):
    """python version of logic for bivariate SPIKE-Sync profile
