...
def _generic_profile_multi(spike_trains, pair_distance_func, indices=None, **kwargs):
    ...
    def divide_and_conquer(pairs1, pairs2):
        ...
        if L1 > 1:
            ...
            dist_prof1 = pair_distance_func(
                spike_trains[pairs1[0][0]], spike_trains[pairs1[0][1]], **kwargs
            )
        L2 = len(pairs2)
        if L2 > 1:
            dist_prof2 = divide_and_conquer(pairs2[: L2 // 2], pairs2[L2 // 2 :])
        else:
            dist_prof2 = pair_distance_func(
                spike_trains[pairs2[0][0]], spike_trains[pairs2[0][1]], **kwargs
            )
    ...
    if L > 1:
        ...
        avrg_dist = divide_and_conquer(
            pairs[: len(pairs) // 2], pairs[len(pairs) // 2 :]
        )
    else:
        avrg_dist = pair_distance_func(
            spike_trains[pairs[0][0]], spike_trains[pairs[0][1]], **kwargs
        )
...
############################################################
def _generic_distance_multi(
    spike_trains, pair_distance_func, indices=None, interval=None, **kwargs
):
    """Internal implementation detail, don't call this function directly,
    ...
    avrg_dist = 0.0
    for i, j in pairs:
        one_dist = pair_distance_func(
            spike_trains[i], spike_trains[j], interval, **kwargs
        )
        avrg_dist += one_dist
...
############################################################
def _generic_distance_matrix(
    spike_trains, dist_function, indices=None, interval=None, **kwargs
):
    ...
    for i, j in pairs:
        d = dist_function(
            spike_trains[indices[i]], spike_trains[indices[j]], interval, **kwargs
        )
