...
class PieceWiseConstFunc:
    ...
    def almost_equal(self, other, decimal=14):
        ...
        """
        eps = 10.0 ** (-decimal)
        return np.allclose(self.x, other.x, atol=eps, rtol=0.0) and np.allclose(
            self.y, other.y, atol=eps, rtol=0.0
        )
    ...
    def integral(self, interval=None):
        ...
        if interval is None:
            ...
            if start_ind > end_ind:
                ...
                a -= (
                    (interval[0] - self.x[end_ind]) + (self.x[start_ind] - interval[1])
                ) * self.y[end_ind]
            else:
    ...
    def add(self, f):
        ...
        try:
            from .cython.cython_add import (
                add_piece_wise_const_cython as add_piece_wise_const_impl,
            )
        except ImportError:
            ...
            from .cython.python_backend import (
                add_piece_wise_const_python as add_piece_wise_const_impl,
            )
